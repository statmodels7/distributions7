...
#' @description
#' Closed form. With \eqn{F = 1 - e^{-q/\mu}},
#' \eqn{\partial F/\partial\mu = -(q/\mu) f} and
#' \eqn{\partial^2 F/\partial\mu^2 = (q/\mu^2) f (2 - q/\mu)}.
...
#' @keywords internal
S7::method(distrib_grad_cdf, ExponentialDistrib) <-
  function(distrib, q, theta, lower.tail = TRUE, log = TRUE) {
    mu <- theta[[1]]
    f <- distrib_pdf(distrib, q, theta)
    d1 <- list(mu = -(q / mu) * f * (q > 0))
    cdf_tail_scale(distrib, distrib_cdf(distrib, q, theta), d1, NULL,
                   lower.tail, log)
  }

#' @rdname distrib_grad_cdf.ExponentialDistrib
#' @name distrib_hess_cdf.ExponentialDistrib
#' @keywords internal
S7::method(distrib_hess_cdf, ExponentialDistrib) <-
  function(distrib, q, theta, lower.tail = TRUE, log = TRUE) {
    mu <- theta[[1]]
    f <- distrib_pdf(distrib, q, theta)
    pos <- q > 0
    d1 <- list(mu = -(q / mu) * f * pos)
    d2 <- list(mu_mu = (q / mu^2) * f * (2 - q / mu) * pos)
    cdf_tail_scale(distrib, distrib_cdf(distrib, q, theta), d1, d2,
                   lower.tail, log)
  }
...
#'
#' @description
#' Closed form at both orders. With \eqn{u = (q/\mu)^\sigma},
#' \eqn{L = \log(q/\mu)} and \eqn{F = 1 - e^{-u}}, every derivative of
#' \eqn{F} follows from those of \eqn{u}, which are elementary:
#' \eqn{\partial u/\partial\mu = -\sigma u/\mu} and
#' \eqn{\partial u/\partial\sigma = uL}.
#'
...
#' @keywords internal
weibull_cdf_deriv <- function(distrib, q, theta, order) {
  mu <- theta[[1]]
  sg <- theta[[2]]
  pos <- q > 0
  r <- ifelse(pos, q / mu, 1)      # a placeholder where the derivative is 0
  u <- r^sg
  L <- base::log(r)
  e <- exp(-u)
  if (order == 1L) {
    return(stats::setNames(
      list(-(sg * u / mu) * e * pos, u * L * e * pos), distrib@params
    ))
  }
  d_mm <- e * (u / mu^2) * (sg * (sg + 1) - sg^2 * u) * pos
  d_ss <- e * u * L^2 * (1 - u) * pos
  d_ms <- e * (u / mu) * (sg * u * L - 1 - sg * L) * pos
  stats::setNames(list(d_mm, d_ss, d_ms), hess_names(distrib@params))
}

#' @title Weibull Log-CDF Derivatives
#' @name distrib_grad_cdf.Weibull1Distrib
#' @description Closed form; see \code{\link{weibull_cdf_deriv}}.
...
#' @seealso \code{\link{weibull1_distrib}}
#' @keywords internal
S7::method(distrib_grad_cdf, Weibull1Distrib) <-
  function(distrib, q, theta, lower.tail = TRUE, log = TRUE) {
    cdf_tail_scale(distrib, distrib_cdf(distrib, q, theta),
                   weibull_cdf_deriv(distrib, q, theta, 1L), NULL,
                   lower.tail, log)
  }

#' @rdname distrib_grad_cdf.Weibull1Distrib
#' @name distrib_hess_cdf.Weibull1Distrib
#' @keywords internal
S7::method(distrib_hess_cdf, Weibull1Distrib) <-
  function(distrib, q, theta, lower.tail = TRUE, log = TRUE) {
    cdf_tail_scale(distrib, distrib_cdf(distrib, q, theta),
                   weibull_cdf_deriv(distrib, q, theta, 1L),
                   weibull_cdf_deriv(distrib, q, theta, 2L),
                   lower.tail, log)
  }
