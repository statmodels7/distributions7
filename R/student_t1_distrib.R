...
#' expected third derivatives have no closed form, so `expected = TRUE` falls
#' back to the numerical [expectation()] of the observed derivatives.
#' @param distrib A `StudentT1Distrib` object.
#' @param y A numeric vector of observations.
#' @param theta A list containing the parameters `mu`, `sigma` and `nu`.
#' @param expected Logical; if `TRUE`, returns the (numerically integrated) expected third derivatives.
#' @param threads How many threads the kernel may use; below the measured
#'   internal threshold it stays sequential whatever the count says.
...
#' expected fourth derivatives have no closed form, so `expected = TRUE` falls
#' back to the numerical [expectation()] of the observed derivatives.
#' @param distrib A `StudentT1Distrib` object.
#' @param y A numeric vector of observations.
#' @param theta A list containing the parameters `mu`, `sigma` and `nu`.
#' @param expected Logical; if `TRUE`, returns the (numerically integrated) expected fourth derivatives.
#' @param threads How many threads the kernel may use; below the measured
#'   internal threshold it stays sequential whatever the count says.
...
#' \deqn{f(y; \mu, \sigma, \nu) = \dfrac{\Gamma\left(\dfrac{\nu+1}{2}\right)}{\sigma\sqrt{\nu\pi}\,\Gamma\left(\dfrac{\nu}{2}\right)} \left(1 + \dfrac{(y-\mu)^2}{\nu\sigma^2}\right)^{-\dfrac{\nu+1}{2}}}
#'
#' **Cumulative distribution function** (\eqn{T_\nu} the standard t CDF):
#' \deqn{F(q; \mu, \sigma, \nu) = T_\nu\!\left(\dfrac{q-\mu}{\sigma}\right)}
#'
#' **Quantile function:**
#' \deqn{Q(p; \mu, \sigma, \nu) = \mu + \sigma\,T_\nu^{-1}(p)}
...
#'
#' **Expected Hessian** (\eqn{\psi_1} the trigamma function; \eqn{\mu} is orthogonal to \eqn{\sigma, \nu}):
#' \deqn{\mathbb{E}\left[\dfrac{\partial^2 \ell}{\partial \mu^2}\right] = -\dfrac{\nu+1}{\sigma^2(\nu+3)}, \qquad
#'       \mathbb{E}\left[\dfrac{\partial^2 \ell}{\partial \sigma^2}\right] = -\dfrac{2\nu}{\sigma^2(\nu+3)}}
#' \deqn{\mathbb{E}\left[\dfrac{\partial^2 \ell}{\partial \nu^2}\right] = \dfrac{1}{4}\left[\psi_1\left(\dfrac{\nu+1}{2}\right) - \psi_1\left(\dfrac{\nu}{2}\right)\right] + \dfrac{\nu+5}{2\nu(\nu+1)(\nu+3)}, \qquad
#'       \mathbb{E}\left[\dfrac{\partial^2 \ell}{\partial \sigma\,\partial \nu}\right] = \dfrac{2}{\sigma(\nu+1)(\nu+3)}}
#' The observed Hessian is available via [distrib_hessian.StudentT1Distrib()].
#'
#' **Moments** (defined for \eqn{\nu} large enough): mean \eqn{\mu} (\eqn{\nu>1}),
