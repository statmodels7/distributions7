...
#' @name LaplaceDistrib
#'
#' @description A subclass of `continuous_distrib` representing the Laplace
#' (double-exponential) distribution. It is the canonical example of a distribution
#' whose log-likelihood is **not differentiable** in its location parameter.
#' @inheritParams distrib
#' @return An object of class `LaplaceDistrib`.
...
#' to \eqn{\mu} is **zero** almost everywhere:
#'
...
#' cannot update \eqn{\mu}; use [distrib_expected_hessian()] (Fisher
...
#' information *is* whenever the score exists, whether or not the identity
...
#' everywhere (observed, or expected when `expected = TRUE`). With
...
#' one the gradient carries, and `params_smooth` records it.
#' @param distrib A `LaplaceDistrib` object.
#' @param y A numeric vector of observations.
#' @param theta A list containing the parameters `mu` and `sigma`.
...
#' notation of [distrib_deriv3.LaplaceDistrib()]: the non-zero
...
#' parameter (\eqn{\mu}); see **Details** for how the package handles this.
#'
...
#'       \dfrac{\partial^2 \ell}{\partial \sigma^2} = \dfrac{\sigma - 2|y-\mu|}{\sigma^3}}
#'
#' **Expected Hessian** (Fisher information from the score variance; see below):
...
#'
#' **Moments:** mean \eqn{\mu}, variance \eqn{2\sigma^2}, skewness 0, excess kurtosis 3.
#'
