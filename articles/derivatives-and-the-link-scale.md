...
log-likelihood. This vignette explains what they are, the two scales you
can ask for them on, and why the second scale — the link scale — is the
one an optimiser wants.

## The derivatives of the log-likelihood

For one observation, write $`\ell(\theta) = \log f(y; \theta)`$. The
package gives you its derivatives with respect to the parameters, per
...
`scale = "parameter"`, is what you have seen: derivatives with respect
to $`\theta`$ on its natural, constrained scale.
...
The information transforms consistently. Because the score has zero
expectation, the expected information on the link scale is the sandwich
$`\operatorname{diag}(g') \, \mathcal{I} \, \operatorname{diag}(g')`$;
for the log link on $`\sigma`$ that multiplies the $`\sigma\sigma`$
entry by $`\sigma^2`$:
...
If you write your own analytical derivative method, its signature
**must** include the `scale` argument, even if the method only ever
