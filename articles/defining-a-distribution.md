...
**Fisher information** computed from the score variance (the first
Bartlett identity, $`I=\mathbb{E}[\nabla\ell\,\nabla\ell^\top]`$) rather
than from $`-\mathbb{E}[H]`$. The two coincide for regular models, but
only the score-variance form stays valid when the second Bartlett
identity fails, giving the correct information $`1/b^2`$ for the Laplace
location where $`-\mathbb{E}[H]`$ would give a degenerate zero:
