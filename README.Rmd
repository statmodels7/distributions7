...
the package, one name per parametrisation where a family has several --
...
how to optimise, and it takes either an optimiser of that package, which
...
Those free values are coordinates an optimiser moves in, not quantities anybody
...
log scale, a correlation on Fisher's *z*. A precision parametrisation reports
...
returns every vector of counts summing to the size, so its normalisation and
