...
would happily straddle the jump and return a number for it, which is
worse than refusing. Away from zero the \\1-\pi\\ factor is constant in
\\y\\, so the parent's own derivative is exact and nothing needs
correcting.
