# Gamma Analytical Expected Hessian

Computes the analytical expected Hessian of the Gamma log-density with
respect to the parameters \\\mu\\ and \\\sigma^2\\.

\$\$\mathbb{E}\left\[\dfrac{\partial^2 \ell}{\partial \mu^2}\right\] =
\dfrac{3\sigma^2 -
4\mu^2\psi_1\left(\dfrac{\mu^2}{\sigma^2}\right)}{(\sigma^2)^2}\$\$
\$\$\mathbb{E}\left\[\dfrac{\partial^2 \ell}{\partial
(\sigma^2)^2}\right\] =
-\dfrac{\mu^2\left(\mu^2\psi_1\left(\dfrac{\mu^2}{\sigma^2}\right) -
\sigma^2\right)}{(\sigma^2)^4}\$\$
\$\$\mathbb{E}\left\[\dfrac{\partial^2 \ell}{\partial \mu \partial
\sigma^2}\right\] =
\dfrac{2\mu\left(\mu^2\psi_1\left(\dfrac{\mu^2}{\sigma^2}\right) -
\sigma^2\right)}{(\sigma^2)^3}\$\$

## Arguments

- distrib:

  A `Gamma2Distrib` object.

- y:

  A numeric vector of observations.

- theta:

  A list containing the parameters `mu` and `sigma2`.

## Value

A list containing the vectors of expected second derivatives.

## See also

[`gamma2_distrib`](https://statmodels7.github.io/distributions7/reference/gamma2_distrib.md)
