...
test_that("all transformers produce correctly normalized densities", {
  ...
  cases <- list(
    log_gamma = transformation(gamma2_distrib(), log_transform()),
    inv_gamma = transformation(gamma2_distrib(), inverse_transform()),
    sqrt_gamma = transformation(gamma2_distrib(), sqrt_transform()),
    power2_gamma = transformation(gamma2_distrib(), power_transform(2)),
    bc_gamma = transformation(gamma2_distrib(), bc_transform(0.5)),
    softplus_gamma = transformation(gamma2_distrib(), softplus_transform()),
    asinh_gauss = transformation(gaussian1_distrib(), asinh_transform()),
    yj_gauss = transformation(gaussian1_distrib(), yj_transform(0.7)),
    expit_gauss = transformation(gaussian1_distrib(), expit_transform()),
    affine_gauss = transformation(gaussian1_distrib(), affine_transform(1, -2)),
    logit_beta = transformation(beta1_distrib(), logit_transform())
  )
  thetas <- list(
...
test_that("decreasing transformations swap tails coherently", {
  ...
  d2 <- transformation(gaussian1_distrib(), affine_transform(loc = 1, scale = -2))
...
test_that("transformed derivatives delegate correctly (finite differences)", {
  ...
  eh_p <- distrib_expected_hessian(gamma2_distrib(), y, th)
  for (p in names(eh_p)) {
...
test_that("transformed moments are computed numerically", {
  ...
  # E[a + bX] = a + b E[X]
  d2 <- transformation(gaussian1_distrib(), affine_transform(1, 2))
...
test_that("invalid supports and inputs are rejected", {
  expect_error(transformation(gaussian1_distrib(), log_transform()), "not valid")
  expect_error(transformation(gaussian1_distrib(), sqrt_transform()), "not valid")
  expect_error(transformation(poisson_distrib(), log_transform()), "continuous")
  expect_error(transformation(gaussian1_distrib(), "not a transformer"), "transformer")
...
test_that("transformations compose", {
  ...
  d <- transformation(transformation(gaussian1_distrib(), exp_transform()), log_transform())
  th <- list(mu = 0.4, sigma = 1.1)
  y <- c(-1, 0, 2)
...
test_that("a transformed distribution can carry the name it is known by", {
  ...
  plain <- transformation(gamma2_distrib(), inverse_transform())
  expect_identical(plain@distrib_name, "inverse(gamma2)")
  ...
  expect_error(transformation(gamma2_distrib(), inverse_transform(), new_name = ""),
               "single non-empty string")
  expect_error(transformation(gamma2_distrib(), inverse_transform(),
                              new_name = c("a", "b")),
               "single non-empty string")
})
